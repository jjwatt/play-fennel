  ...
  (let [points []]
    (for [degree 0 360 2]
      (let [rad (math.rad degree)
            ...
            noise-x (+ (* cx 0.01) (* cos-val 0.5))
            noise-y (+ (* cy 0.01) (* sin-val 0.5))
            noise-factor (love.math.noise noise-x noise-y)
            variance (* base-radius 1.5 noise-factor)
            current-radius (+ base-radius 1.5 noise-factor)
